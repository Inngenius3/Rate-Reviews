...
export class UserSettingComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    this.listenerFn = this.renderer.listen('document', 'click', (event) => {
      if (event.target.hasAttribute("data-edit-id")) {
        this.router.navigate(["userSetting/edit/" + event.target.getAttribute("data-edit-id")]);
      }
      ...
      if (event.target.hasAttribute("data-permission-id")) {
        this.router.navigate(["userSetting/permissions/" + event.target.getAttribute("data-permission-id")]);
      }
    });
  }
  ...
  getTableData(){
    ...
    this.dtOptions = {
      ...
      // data:null,
      // render: function (data, type, row, meta) {
      //    return '<i class="fas fa-key tx-orange tx-12" role="button" data-permission-id="'+row.id+'" routerLink="/userSetting/permissions"></i>';
      // }},
